...
plurals, and falls back gracefully — from Rust code or from a Tera template.
...
Keep the `Translator` in your app state and look up strings with the
[`ActiveLocale`](middleware.md#locale-aware-middleware) the middleware resolved.

`Translator` itself is **not** `Clone` — it holds `RwLock`s — so share it
behind an `Arc`, which is what the template helpers below take:
...
```rust
// `translator` is the `Arc<Translator>` from above — `register` takes the
// `Arc`, and cloning it is cloning the handle, not the catalogs.
