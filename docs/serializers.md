...
| `writable_fields` | `fn() -> &'static [&'static str]` | Field names accepted on write (excludes `read_only`, `skip`, `method`, `nested`, `many`, `slug`). |
| `validate` | `fn(&self) -> Result<(), FormErrors>` | Runs declared validators. No-op when none are declared; you can override it for cross-field rules. |

There is deliberately **no** `create` / `update` / `save` on the trait — writes
go through the ORM (`model.save(&pool)`), with the serializer supplying
`writable_fields()` and `validate()`.
...
A serializer composes naturally with the ORM and a custom handler — map a row
and emit its JSON:
...
> **Heads-up on `ViewSet::serializer()`.** A `.serializer::<S>()` builder method
> exists and is *intended* to render a ViewSet's list/retrieve responses through
> the serializer's overrides. After the v0.38 tri-dialect refactor, the list
> path projects JSON directly (so it works on every backend) and that per-row
> serializer rendering is **not currently applied** — a ViewSet's output shape
> is controlled by its own `fields` instead (see the
> [ViewSets guide](viewsets.md#response-shape-fields-and-serializers)). To get a
> serializer's exact shape on an endpoint today, map through it in a custom
> handler as above. Standalone use (`from_model` / `to_value`) is unaffected and
...
A serializer also derives `serde::Deserialize`, so in a custom write handler you
can parse a request body straight into it, run `.validate()`, and — on success —
map the data onto a model and `save(&pool)`. `writable_fields()` tells you which
keys the serializer considers writable (it excludes `read_only` / computed /
relational fields), which is handy for filtering an incoming body before
binding. (Note: the auto-generated ViewSet write path uses the ViewSet's own
`fields` list for that filtering, not `writable_fields()`.)
