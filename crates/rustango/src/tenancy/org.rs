...
pub struct Org {
    ...
    // SSO / OpenID Connect config moved off the `Org` row (v0.47): it now
    // lives in the `SsoProvider` (per-tenant) + `SharedSsoProvider`
    // (registry-wide) models, so a tenant can offer MANY providers, all
    // managed from the admin UI. See `crate::admin::sso_provider` and
