//! Postgres-backed job queue — safe pickup across multiple processes.
//!
//! Worker tasks pick the next ready job with
//! `SELECT … FOR UPDATE SKIP LOCKED` so two replicas never grab the
//! same row. Retries land back in the same table with `run_at` pushed
...
//! PgJobQueue::ensure_table(&pool).await?;
//!
//! let queue = Arc::new(
//!     PgJobQueue::with_workers(pool.clone(), 4)
...
//! );
//! CREATE INDEX rustango_jobs_pickup_idx
//!     ON rustango_jobs (run_at)
//!     WHERE locked_at IS NULL;
//! ```
//!
//! Use [`PgJobQueue::ensure_table`] at boot, or roll your own migration
//! that emits the same DDL.
//!
//! ## Lock recovery
//!
//! [`PgJobQueue::reclaim_stuck_jobs`] resets `locked_at = NULL` for any
//! row whose lock is older than the threshold. Call it on a scheduler
//! to recover from worker crashes that left jobs flagged-but-unfinished.
...
///
/// Cheap to clone (everything is `Arc`-wrapped internally). Workers are
/// not spawned until [`PgJobQueue::start`] is called.
...
impl PgJobQueue {
    /// Build a queue with `worker_count` worker tasks. Call
    /// [`Self::start`] to actually spawn them.
    #[must_use]
    ...
    /// Default: 4 workers, 1-second poll interval.
    #[must_use]
    ...
    /// Create the `rustango_jobs` table + pickup index if they don't
    /// exist. Idempotent. Safe to call on every boot.
    ///
    /// # Errors
    /// Returns the underlying sqlx error if the DDL fails (insufficient
    ...
    /// Reset `locked_at = NULL` on any row locked longer than `older_than`.
    /// Call from a scheduled task (e.g. every minute) to recover from
    /// worker crashes that left rows reserved but not finished.
    ///
...
async fn run_one(
    ...
    // Look up the handler. If unregistered, leave the row locked + log; the
    // reclaim_stuck_jobs job will eventually free it. (Better than deleting
    // — could just be a deploy lag where one replica registered the new
    // handler but another hasn't yet.)
    //
    // Snag the static name so we can pass it through to dead_letter without
    // owning a String there (DeadLetter::name is &'static str).
...
// `sqlx::types::chrono::DateTime<Utc>` is referenced by the schema doc
// comment but not used at the type level — keep the import quiet.
const _: fn() = || {
...
#[cfg(test)]
mod tests {
    //! No-DB unit tests. The `PgJobQueue` SQL paths require a live
    //! Postgres — exercise them in `tests/jobs_pg_live.rs` if/when that
    //! exists. These tests just cover the pure-Rust bits.
    ...
    async fn register_lookups_handler_under_static_name() {
        ...
        // lookup_owned must return the static name so dead-letter rows
        // keep `&'static str` after the row name comes back as String.
    ...
    fn dummy_pool() -> PgPool {
        ...
        // construct `PgJobQueue` for non-IO assertions. lazy = true is
        // the key bit; sqlx 0.8 returns the pool immediately and only
        // dials when a query runs.
