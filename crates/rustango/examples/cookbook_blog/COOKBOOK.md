...
│       ├── blog/...
│       └── ...
├── migrations/0002_cookbook_tables.json   // your app's migrations
├── system/migrations/0001_*.json          // framework tables, generated by makemigrations
...
* §5.77 **Extras on tenant users** — three escalating options when the
  ...
  3. **`Cli::user_model::<AppUser>()`** — extras inline on
     ...
     chain `.user_model::<AppUser>()` on `Cli` (or `Builder`). Because
     your `AppUser` *is* the `rustango_users` model, its columns flow
     through the same `makemigrations`/`migrate` engine as everything
     else: the framework's own tables are generated into the project's
     `system/migrations/` folder from the compiled models (there is no
     hand-shipped bootstrap JSON), so `AppUser`'s extra columns land in
     the generated `CREATE TABLE rustango_users`. Caveats:
     - Only one model may claim `table = "rustango_users"`. Register
       `AppUser` *instead of* the framework `User` (don't register
       both) — two models on the same table make `makemigrations`
       ambiguous.
     - Validation (`validate_tenant_user_schema`) rejects a wrong
       table name or a missing required column.
  → option 3 covered by `tenancy::auth::tests::validate_*` in the
    framework crate.
...
otherwise have to learn (`migrate-registry` → `create-operator`
→ `create-tenant` → `create-superuser`).
...
#### MCP is an optional feature — its tables are plug-and-play
...
`mcp` feature. A `#[cfg(feature = "mcp")]`-gated model is compiler-stripped
before the derive macro sees it, so it's simply absent from the model set when
`mcp` is off. This makes the tables **plug-and-play** through the ordinary
migration engine: the framework's own tables are generated into the project's
`system/migrations/` folder from the compiled models, so **enabling** `mcp`
makes the next `makemigrations` emit `CreateTable` for the agent/skill tables,
and **disabling** it emits `DropTable` — no hand-written DDL, no bootstrap
JSON. The same mechanism governs every framework table (`rustango_orgs`,
`rustango_users`, `content_types`, `audit_log`, `permissions`, …): they live
in `system/migrations/`, generated by `makemigrations`, applied by `migrate`.
Your app's own `makemigrations` never emits create/drop for `rustango_*`
tables — the framework owns that prefix and routes it to `system/migrations/`.
