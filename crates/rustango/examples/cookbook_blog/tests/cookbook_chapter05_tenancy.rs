...
async fn provision_two_tenants_then_resolve_and_lazy_pool() {
    ...
    // path (e.g. inventory `apply_all`) without recording them in the
    // system-migration ledger. Reset the registry tables AND both
    // ledgers so `migrate_registry` starts from a clean, in-sync state
    // — the framework's own tables migrate under a dedicated
    // `__rustango_system_migrations__` ledger, separate from the
    // user-app `__rustango_migrations__`.
    for tbl in [
        "rustango_orgs",
        "rustango_operators",
        "rustango_audit_log",
        ...
        let _ = sqlx::query(&format!(r#"DROP TABLE IF EXISTS "{tbl}" CASCADE"#))
            .execute(&registry)
            .await;
    }

    let pools = TenantPools::new(registry.clone());
    let dir = std::env::temp_dir().join(format!("cookbook_ch5_{}", uuid::Uuid::new_v4().simple()));
    std::fs::create_dir_all(&dir).unwrap();
    // `migrate_registry` generates the framework's registry-scope
    // `system/migrations/` from the compiled models (into `dir`) and
    // applies them — no separate bootstrap step.
    tenancy::migrate_registry(&pools, &dir)
        .await
    ...
    for slug in ["cookbook_acme", "cookbook_globex"] {
        ...
        tenancy::manage::api::create_tenant_if_missing(&pools, &registry_url, &dir, slug, opts)
            .await
