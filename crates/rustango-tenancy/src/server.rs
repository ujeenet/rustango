...
use crate::admin::TenantAdminBuilder;
use crate::error::TenancyError;
use crate::operator_console::{self, SessionSecret};
...
async fn shutdown_signal() {
    ...
    tracing::info!(target: "rustango_tenancy::server", "shutdown signal received");
}
...
async fn no_operators_warn<W: Write + Send>(
    ...
    let active: Vec<crate::auth::Operator> = crate::auth::Operator::objects()
        .where_(crate::auth::Operator::active.eq(true))
        .fetch(registry)
        .await?;
    if active.is_empty() {
