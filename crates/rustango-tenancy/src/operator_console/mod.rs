...
//!
//! ```ignore
//! let console = rustango_tenancy::operator_console::router(
//!     registry.clone(),
...
async fn require_session(
        ...
        .await
    {
        ...
        Err(e) => {
            tracing::warn!(target: "rustango_tenancy::operator_console", error = %e, "registry lookup");
...
async fn login_submit(
    ...
    .await
    {
        ...
        Err(e) => {
            tracing::warn!(target: "rustango_tenancy::operator_console", error = %e);
...
async fn orgs_list(
    ...
    let rows: Vec<crate::Org> = match crate::Org::objects().fetch(&state.registry).await {
        Ok(r) => r,
        Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, format!("{e}")).into_response(),
    };
    let view: Vec<_> = rows
